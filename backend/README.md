...
**Tech stack:** Ragie (video-RAG), YouTube/yt-dlp (single video or full playlist), optional Cloudinary (video storage), optional OpenRouter/Llama agent, Cursor (MCP host).
...
- `show_video_tool`: Create a video chunk from a segment (saved to `video_chunks/`, optional Cloudinary upload)
...
- Cloudinary folders (if used) are `users/{user_id}/videos` and `users/{user_id}/chunks`; API returns `secure_url` in `b2_key` for playback.
...
A database stores **user details**, **videos** (with Cloudinary URL in `b2_key` when used), and **chats/messages**. The schema is equivalent to what you’d model in Prisma; this project uses **SQLAlchemy** (Python) with SQLite by default or PostgreSQL via `DATABASE_URL`.
...
| **videos** | `id`, `user_id`, `filename`, `b2_key` (Cloudinary URL when set), `source`, `source_url`, `created_at` |
...
| GET | `/api/videos` | List this user’s videos (filename, b2_key = Cloudinary URL when set, source, source_url). |
...
On **ingest (YouTube or directory)**, videos are written to the DB with `filename`, `b2_key` (Cloudinary secure_url when Cloudinary is configured), and `source`/`source_url`. Your frontend can list them via `GET /api/videos` and use `b2_key` as the playback URL when present.
